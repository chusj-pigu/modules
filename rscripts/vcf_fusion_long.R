...
max_ann <- max(str_count(vcf_fusion$ANN, ",")) + 1
vcf_fusion <- vcf_fusion %>% separate(ANN, into = paste0("ann", 1:max_ann), sep = ",") %>%
  ...
  select(CHROM, POS, CHR2, END, SVTYPE, Annotation, Gene_name, SVLEN, `HGVS.c`, `HGVS.p`, Annotation_impact, Allele, LOF_Number_of_transcripts_in_gene, LOF_Percent_of_transcripts_affected, ID) %>%
  rename(BREAKPOINT2=END)

  
# Replace %3 html characters to ;
vcf_fusion$HGVS.c <- gsub("%3", ";", vcf_fusion$HGVS.c)
vcf_fusion$HGVS.p <- gsub("%3", ";", vcf_fusion$HGVS.p)
